...
            AsyncObject ao = new AsyncObject(1);
        ...
        private void RecieveQueueData(IAsyncResult ar)
        {
        ...
        private void handleDataReceive(IAsyncResult ar)
        {
            ...
            if (recvBytes > 0)
            {
                ...
                String splitString = Encoding.Unicode.GetString(msgByte);
                String[] split_data = splitString.Split(' ');

                if (split_data[0].Equals("SEND"))
                {   
                    queue.Enqueue(split_data[1]);
                  
                }else if (split_data[0].Equals("RECEIVE"))
                {
                ...
                // 받은 메세지를 출력
                //queue.Enqueue(Encoding.Unicode.GetString(msgByte));
